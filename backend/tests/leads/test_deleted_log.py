...
def test_clear_junk_logs_reason_junk(tmp_path):
    ...
    log = store.deleted_log()
    assert len(log) == 1
    assert log[0]["email"] == "bob@dead.com"
    assert log[0]["reason"] == "junk"
...
def test_delete_returns_false_and_no_double_log(tmp_path):
    ...
    log = store.deleted_log()
    assert len(log) == 1
    assert log[0]["email"] == "twice@acme.com"
    assert log[0]["reason"] == "manual"  # original reason preserved
